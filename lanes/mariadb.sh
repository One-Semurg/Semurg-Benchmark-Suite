#!/usr/bin/env bash
# mariadb lane -- runs the equal-answer workload against mariadb in Docker on YOUR box.
...
set -euo pipefail
echo "SKIP mariadb reason=engine-lane-scaffolded-run-with:'semurg-arena run mariadb'-after-kit-v1.1(see README);honest-no-fake-number"
