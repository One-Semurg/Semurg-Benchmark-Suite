#!/usr/bin/env bash
# redis lane -- runs the equal-answer workload against redis in Docker on YOUR box.
...
set -euo pipefail
echo "SKIP redis reason=engine-lane-scaffolded-run-with:'semurg-arena run redis'-after-kit-v1.1(see README);honest-no-fake-number"
