#!/usr/bin/env bash
# questdb lane -- runs the equal-answer workload against questdb in Docker on YOUR box.
...
set -euo pipefail
echo "SKIP questdb reason=engine-lane-scaffolded-run-with:'semurg-arena run questdb'-after-kit-v1.1(see README);honest-no-fake-number"
