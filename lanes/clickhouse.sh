#!/usr/bin/env bash
# clickhouse lane -- runs the equal-answer workload against clickhouse in Docker on YOUR box.
...
set -euo pipefail
echo "SKIP clickhouse reason=engine-lane-scaffolded-run-with:'semurg-arena run clickhouse'-after-kit-v1.1(see README);honest-no-fake-number"
