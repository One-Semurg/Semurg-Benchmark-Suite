#!/usr/bin/env bash
# timescale lane -- runs the equal-answer workload against timescale in Docker on YOUR box.
...
set -euo pipefail
echo "SKIP timescale reason=engine-lane-scaffolded-run-with:'semurg-arena run timescale'-after-kit-v1.1(see README);honest-no-fake-number"
