...
DATA="${ARENA_DATA:?}"; IMG="${TS_IMAGE:-timescale/timescaledb:latest-pg16}"; C="arena_ts_$$"
st="$(arena_docker_status)"
[ "$st" = ok ] || { echo "SKIP timescale reason=docker-$st fix=[$(arena_docker_fix "$st")]"; exit 0; }
...
if ! docker run -d --rm --label "$ARENA_LABEL=1" --name "$C" -e POSTGRES_PASSWORD=a -e POSTGRES_DB=arena "$IMG" >/tmp/arena_ts_$$.err 2>&1; then
  echo "SKIP timescale reason=image-pull-or-start-failed detail=[$(tr -d '\n' </tmp/arena_ts_$$.err | tail -c 90)] fix=[check network/registry, disk space, or: docker pull $IMG]"; rm -f /tmp/arena_ts_$$.err; exit 0
fi
rm -f /tmp/arena_ts_$$.err
ready=0
...
[ "$ready" = 1 ] || { echo "SKIP timescale reason=engine-not-ready-in-60s(container crashed or slow; logs: docker logs $C)"; exit 0; }
...
run_ts(){
  ...
  echo "LANE=timescale LOAD_MS=$load_ms Q1_MS=$q1ms Q2_MS=$q2ms Q3_MS=$q3ms ANSWER_HASH=$(hash_answer "$q1|$q2|$q3")"
}
run_ts || echo "LANE=timescale status=FAILED reason=query-error(engine ran but a step failed; logs: docker logs $C)"
