...
export OLAP_EXS="${OLAP_EXS:-$HERE/semurg_olap.exs}"
...
if [ "${ARENA_REQUIRE_SHIPPED:-0}" = 1 ]; then
  case "$sline" in
    *release-not-installed*)
      echo
      echo "GATE FAIL: semurg olap lane skipped with release-not-installed under ARENA_REQUIRE_SHIPPED=1 (r11 GATE)"
      rm -rf "$OLAP_SCRATCH" 2>/dev/null || true
      exit 3;;
  esac
  # G3: a Semurg-vs-DuckDB equal-answer MISMATCH is a WRONG aggregate, never a green board -- fail the
  # gate. (Both engines finished, both answers present, and they disagree.)
  if [ "$sstat" = ok ] && [ "$dstat" = ok ] && [ -n "$sans" ] && [ -n "$dans" ] && [ "$dans" != "$sans" ]; then
    echo
    echo "GATE FAIL: olap equal-answer MISMATCH (semurg=$sans duckdb=$dans) under ARENA_REQUIRE_SHIPPED=1 (r11 GATE)"
    rm -rf "$OLAP_SCRATCH" 2>/dev/null || true
    exit 3
  fi
fi
